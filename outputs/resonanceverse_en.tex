...
Relations are modeled as temporal integrals coupling contextual weights and differences. Resonance requires the joint satisfaction of semantic, uncertainty, and value constraints $\mathcal{C}_M(t), \mathcal{C}_U(t), \mathcal{C}_V(t)$; this paper articulates how pragmatic stability emerges without convergence to semantic identity.
...
Finally, residual semantic differences that do not disappear over time are introduced. To avoid confusion with the graph $G(t)$, residual amplitudes are denoted by $\Gamma$ rather than $G$. The finite-tail residual is defined as
\[
\Gamma_{ij}(T)
:=
\sup_{t \ge T}
\|\Delta M_{ij}(t)\|,
\]
and its asymptotic counterpart is
\[
\Gamma_{ij}^{\infty}
:=
\limsup_{t \to \infty}
...
The quantity $\Gamma_{ij}(T)$ captures the maximal residual amplitude after time $T$, while $\Gamma_{ij}^{\infty}$ captures the long-run upper residual. These quantities express the fact that meaning is never fully unified, and they provide a way to distinguish finite-horizon residual control from asymptotic non-collapse.
...
To characterize this non-convergent stability more precisely, this paper introduces the concept of \textit{resonance}. The symbols $\mathcal{C}_M(t), \mathcal{C}_U(t), \mathcal{C}_V(t)$ denote predicates that assign truth values at each time; they are not abbreviations for the semantic state vector $M_i(t)$. Their formal definitions follow.

\begin{definition}[Constraint Functions $\mathcal{C}_M, \mathcal{C}_U, \mathcal{C}_V$]
Let $\theta_M, \theta_U, \theta_V > 0$ be domain-specific thresholds.

\textbf{(i) Semantic variance constraint.}
\[
\mathcal{C}_M(t) :\Leftrightarrow
\operatorname{Var}_{(i,j)\in E(t)}\!\left(\|\Delta M_{ij}(t)\|\right) < \theta_M,
\]
where the variance is taken over the scalar edge norms $\|\Delta M_{ij}(t)\|$.
...
\[
D_U(t) := \max_{(i,j) \in E(t)} |U_i(t) - U_j(t)|, \quad \mathcal{C}_U(t) :\Leftrightarrow D_U(t) < \theta_U.
\]
...
\[
D_V(t) := \max_{(i,j) \in E(t)} d_{\cos}(V_i(t), V_j(t)), \quad \mathcal{C}_V(t) :\Leftrightarrow D_V(t) < \theta_V.
\]
\end{definition}
...
The cosine distance $d_{\cos}$ is undefined when $\|V_i\| = 0$. Such agents---those lacking evaluative direction---are excluded from the $\mathcal{C_V}$ evaluation. Formally, $D_V(t)$ is computed over the restricted edge set $E_V(t) := \{(i,j) \in E(t) : \|V_i(t)\| > 0 \text{ and } \|V_j(t)\| > 0\}$. If $E_V(t) = \emptyset$, the condition $C_V(t)$ is vacuously true.
\end{remark}

\begin{definition}[Resonance]
A multi-agent system is in \textit{resonance} at time $t$ if and only if the semantic variance condition ($\mathcal{C}_M$), uncertainty alignment condition ($\mathcal{C}_U$), and value direction condition ($\mathcal{C}_V$) are simultaneously satisfied. Resonance is the state in which semantic differences persist but are coherently constrained across multiple dimensions.
\end{definition}

It is crucial that alignment here does not mean complete identity. $\mathcal{C}_M$ requires that semantic differences be controlled in terms of variance and thresholds, not eliminated. Likewise, $\mathcal{C}_U$ refers to the degree of shared uncertainty, and $\mathcal{C}_V$ to alignment of evaluative standards or directions of judgment.
...
Considering the long-term behavior of semantic differences, there remains a long-term amplitude that does not disappear. Taking the norm first, define the finite-tail residual and the asymptotic residual by
\[
\Gamma_{ij}(T)
:=
\sup_{t \ge T}
...
\Gamma_{ij}^{\infty}
=
\lim_{T\to\infty}\Gamma_{ij}(T)
=
\inf_{T\ge 0}
\sup_{t\ge T}
\|\Delta M_{ij}(t)\|.
\]
To avoid the earlier draft problem of mixing $\limsup$ with finite time $t$, amplitude is always written through $\|\cdot\|$, $h_{ij}$, and the $\Gamma$ notation.
...
Thus, resonance is positioned as a condition for stability under non-convergence. Meaning can remain structurally stable while preserving difference without converging. This stability arises through the simultaneous action of the constraints $\mathcal{C}_M, \mathcal{C}_U, \mathcal{C}_V$.
...
\section{Theoretical Properties and Modeling Conditions}

This section briefly lists the mathematical foundations on which Sections 2 through 4 rely. Full proofs are not developed here; rigorous development is left to future work. The term ``Theorem'' is avoided, and claims that still require further assumptions are explicitly labeled as Proposition or Phenomenological Principle. In particular, the boundedness of the update term $f$ alone does not imply boundedness of trajectories. The central assumptions of this section therefore concern bounded updates, bounded interaction degree, an absorbing semantic region, and non-collapse.

\begin{assumption}[Bounded Interaction Field]
There exists $C_f>0$ such that
\[
\|f(\alpha,x)\| \le C_f
\quad
\text{for all } \alpha\in[0,1],\ x\in\mathbb{R}^d.
\]
\end{assumption}

\begin{assumption}[Bounded Interaction Degree]
There exists $D<\infty$ such that
\[
\deg(i,t) \le D
\quad
\text{for all } i,t.
...
\begin{assumption}[Absorbing Semantic Region]
There exists a bounded set
\[
\mathcal{B}\subset\mathbb{R}^d
\]
such that every admissible trajectory eventually enters and remains in $\mathcal{B}$.
\end{assumption}

\begin{assumption}[Non-Collapse Condition]
For the relevant interaction pair $(i,j)$,
\[
\limsup_{t\to\infty}\|\Delta M_{ij}(t)\|>0.
\]
\end{assumption}
...
Under the assumptions above and the non-convergence condition
\[
\limsup_{t \to \infty} \|\Delta M_{ij}(t)\| > 0
\]
...
Under the expanded-region assumption, perturbations are less likely to appear as anomalies and are instead layered onto existing fluctuations. The trajectory is globally constrained in $B'$, and resonance as structure is evaluated only by threshold exceedances.
\end{phenomenological}

\begin{proposition}[Descriptive Meaning of Resonant States]
If semantic norm variance $\operatorname{Var}_{(i,j)\in E(t)}(\|\Delta M_{ij}(t)\|)$ and the uncertainty and value divergence indicators $D_U, D_V$ are each below their thresholds, then $\mathrm{Resonance}(t)$ is satisfied. This is a direct restatement of the definition in Section 4; its existence becomes nontrivial only under concrete assignments of model coefficients.
\end{proposition}
...
\subsection{Phase 1: Two-Agent Illustrative Dynamics}
...
\subsection{Phase 3: Resonance-Condition Diagnostic Run}
...
\textbf{Results:}
\begin{itemize}
\item \textbf{Resonance attainment:} 98.6\% of timesteps satisfy $\mathcal{C}_M \land \mathcal{C}_U \land \mathcal{C}_V$.
\item \textbf{Lyapunov descent:} $V(t) = \frac{1}{2}\sum_i \|M_i\|^2$ decreases by 58.9\%.
\item \textbf{Condition independence (dynamical):} Correlation between $\mathcal{C_M}$ and $\mathcal{C_V}$: 0.609.
...
\section{Philosophical and Institutional Discussion}
\subsection{Methodological Analogy with Formal Incompleteness}
...
Section 6 presented illustrative numerical simulations across four phases. The experiments demonstrate that, under explicitly specified dynamics, one can construct bounded non-convergent behavior, observe institutional constraint effects, obtain resonance-like satisfaction of $\mathcal{C}_M \land \mathcal{C}_U \land \mathcal{C}_V$, and formulate falsifiability-oriented diagnostics. These results support the coherence of the proposed modeling framework, but they do not constitute a universal empirical proof of the non-convergence axiom.
